...
const LoginForm = ({ onChangeImage }) => {
  ...
  const mutationGoogle = useMutationHooks((data) =>
    UserServices.loginUserGoogle(data)
  );
    ...
    if (mutationGoogle.isSuccess) {
      if (mutationGoogle.data.status === "Error") {
        ...
        if (mutationGoogle.data?.access_token) {
          ...
          if (decode?.id) {
            handleGetDetailsUser(decode?.id, mutationGoogle.data?.access_token);
          }
        } else {
          message.error();
        }
    ...
    <>
      <div>
        ...
        <form onSubmit={handleSubmitRegisterLoggin}>
          ...
          {errors.email && (
            <small className="error-message-l">{errors.email}</small>
          )}
          <div
            className={`input-container-L-S ${
          ...
          >
            ...
            />
            {errors.password && (
              <small className="error-message-l">{errors.password}</small>
            )}
          </div>
