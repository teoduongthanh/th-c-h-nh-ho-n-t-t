            ...
            <div className="py-20 p-4">
                ...
                <ul className="grid grid-cols-4 gap-2">
                    {exercises.map((exercise, index) => (
                        <li className="bg-slate-50 border-2 px-2 hover:bg-green-50" key={exercise.id}><a href={`/exercise-segments/${exercise.id}`} >{exercise.title}</a></li> // Render exercise titles
                    ))}
                </ul>
            </div>
        </div>
